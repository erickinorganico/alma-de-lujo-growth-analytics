...
- **D-03 — Live native evidence stays distinct from deterministic CI.** The release run executes actual native Codex analyst tasks and a different independent reviewer according to `agents/RUN-NATIVE-CYCLE.md`. Full requests, responses and traces stay in ignored `.local/`; the public acceptance record contains only safe runtime identifiers, roles/models, timestamps, hashes, challenge disposition and final process state. Fixture responses test validators but never count as live inference. Source: `AGENTS.md`, REL-01 and `05-RESEARCH.md`.
...
- **D-06 — Reproducibility starts from a committed archive.** Build and test the candidate from `git archive` of the exact commit, install only pinned existing dependencies in a disposable environment, and run the deterministic acceptance, source-hash, privacy, portal/link and package checks on Windows and Ubuntu GitHub Actions. The project wrapper `scripts/github-personal.ps1` is the only GitHub CLI route and must validate `erickinorganico`. Source: REL-03, `AGENTS.md` and `05-RESEARCH.md`.
...
- **D-08 — Release provenance is non-recursive and exact.** The tagged commit produces `Alma_OS_v1.0.0.zip`; a checksum sidecar records its SHA-256, and a separate acceptance sidecar binds that ZIP hash, tag, commit, CI runs, native summary, Excel evidence, archive proof and gate results. The ZIP contains the runbook and an inner acceptance summary that does not claim its own outer ZIP hash. All public bytes are blank or explicitly synthetic. Source: REL-04 and `05-RESEARCH.md`.
