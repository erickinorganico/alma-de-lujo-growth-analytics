...
- An authorized analysis cycle follows `agents/RUN-NATIVE-CYCLE.md`: actual native execution, analyst response validation, and a distinct independent reviewer. Deterministic scripts do not count as agent runs. [VERIFIED: `AGENTS.md`, `agents/RUN-NATIVE-CYCLE.md`]
...
Use the v0.2 and v0.3 verifiers unchanged as regression gates, then add one v1 acceptance runner that creates two **different** synthetic weekly cuts in temporary private directories and checks the complete chain from source pack to reviewed decision and next-cut closure. An identical replay is a separate idempotence control; it cannot stand in for week two. Keep native execution evidence distinct from deterministic replay, including actual task IDs, dispatch receipts, response hashes, a different reviewer identity and a challenge trace. [VERIFIED: `scripts/verify_v2.py`, `agents/RUN-NATIVE-CYCLE.md`, `.planning/ROADMAP.md`; RECOMMENDATION]
...
|---|---|---|---|
| Two-week cut/decision proof | Local CLI/test runner | SQLite workspace | The CLI orchestrates immutable cuts and native evidence; SQL owns exact facts and reconciliation. [VERIFIED: `.planning/PROJECT.md`, `agents/RUN-NATIVE-CYCLE.md`] |
...
2. Dispatch real native analysis for the bound synthetic week 1 evidence and a **different** reviewer. Retain request, response, actual task/dispatch receipt, reviewer challenge, validated terminal packet and SHA-256 for each. A prepared request or synthetic replay alone is not live native analysis. [VERIFIED: `agents/RUN-NATIVE-CYCLE.md`, `.planning/REQUIREMENTS.md`]
...
Use `git archive` from the candidate commit/tag into a disposable directory, then run the build and checks there using documented dependencies. `git archive` uses a committed tree, so untracked `.planning/` and local files in the development checkout are absent unless committed; that is a useful release test. Compare generated manifests, canonical source/mart exports and expected source hashes to the candidate receipts. Do not require byte-identical freshly built SQLite database files: v0.2 documents that canonical data/exports can match while SQLite binaries differ. [CITED: https://git-scm.com/docs/git-archive; VERIFIED: current `git status --short`, `docs/RELEASE.md`; RECOMMENDATION]
...
For any delivered formula or rendered workbook change, regenerate synthetic adversarial probes, recalculate those and the blank/example deliverables in a **new** desktop Excel instance, save/normalize metadata, and record engine/version/build plus post-save SHA-256. Then run `verify_client_v3.py check` against the exact delivered files and receipt. Export and inspect complete `INICIO`, `DECISIONES` and `FLUJO_13_SEMANAS` pages for both delivered books; record inspected PDF hashes and a human visual disposition tied to workbook hashes. Rebuild the ZIP after the final Excel save and verify its workbook member bytes match the receipt and release manifest. [VERIFIED: `docs/CLIENT-OPERATIONS-v3.md`, `scripts/recalculate_client_excel.ps1`, `scripts/verify_client_v3.py`, `docs/CLIENT-SYSTEM-ACCEPTANCE.md`; RECOMMENDATION]
...
| Native cognition/review | Actual native Codex task bridge and recorded dispatches | Prepared JSON or deterministic replay cannot prove an analyst or reviewer actually ran. [VERIFIED: `agents/RUN-NATIVE-CYCLE.md`] |
...
| Native Codex runtime | Live analyst/reviewer proof | Available to the orchestrating Codex workflow, not callable by deterministic Python alone. [VERIFIED: `agents/RUN-NATIVE-CYCLE.md`] | Keep `WAITING_AGENT`/`BLOCKED` until real dispatch evidence exists. [VERIFIED: `agents/RUN-NATIVE-CYCLE.md`] |
...
Produce a reviewable acceptance record keyed by candidate commit and release ZIP SHA-256, with: exact test/CI commands and results on Windows/Linux; week 1/2 input hashes and cut IDs; source/mart reconciliation receipts; actual native analyst/reviewer task and response hashes; closure or stale evidence; negative-control outcomes; workbook oracle/Excel/PDF hashes if triggered; clean-archive build results; package member allowlist/hash and local-link audit; privacy audit with its limits; runbook and versioned checksum. Mark unavailable gates BLOCKED, and retain external `EXT-01..03` as UNKNOWN/REVIEW. The package should contain only reviewed public blank/synthetic artifacts; private workspaces, filled books, native responses and credentials remain out of it. [VERIFIED: `.planning/REQUIREMENTS.md`, `AGENTS.md`, `agents/RUN-NATIVE-CYCLE.md`, `docs/CLIENT-SYSTEM-ACCEPTANCE.md`; RECOMMENDATION]
...
1. What exact Phase 1–4 command and schema names will the final acceptance runner call? Freeze them from implemented outputs before writing Phase 5 fixtures. [VERIFIED: `.planning/ROADMAP.md` shows earlier phases pending; RECOMMENDATION]
2. Will the final release change any formula, layout, or delivered workbook bytes? Compare candidate to the v0.3 hash/formula baseline, then trigger new Excel and visual evidence as required. [VERIFIED: `evidence/v0.3/excel/excel-recalculation.json`, `.planning/REQUIREMENTS.md`; RECOMMENDATION]
3. Can the clean archive build and full CI matrix finish within the current 15-minute job limit? Measure on the implemented v1 candidate; increase justified timeout if needed, without removing checks. [VERIFIED: `.github/workflows/verify.yml`, `.planning/codebase/CONCERNS.md`; RECOMMENDATION]

## Sources

**Primary project evidence:** `.planning/ROADMAP.md`, `.planning/REQUIREMENTS.md`, `.planning/PROJECT.md`, `AGENTS.md`, `docs/CLIENT-SYSTEM-ACCEPTANCE.md`, `docs/CLIENT-OPERATIONS-v3.md`, `agents/RUN-NATIVE-CYCLE.md`, `.github/workflows/verify.yml`, `scripts/verify_v2.py`, `scripts/check_scope_v2.py`, `scripts/verify_client_v3.py`, `scripts/recalculate_client_excel.ps1`, `scripts/package_client.py`, `scripts/audit_release.py`, `tests/test_client_system.py`, `tests/test_client_review.py`, `tests/test_process_engine.py`, `evidence/v0.3/excel/excel-recalculation.json`. [VERIFIED: codebase inspection]
