...
- Before the public kit is accepted, render the public demo and a safe synthetic-current private fixture through `file://` at desktop and 390px narrow widths plus print/PDF. Inspect every captured page and bind exact HTML/image/PDF hashes, browser/version, viewport, timestamp and per-surface disposition in `evidence/v1.0/portal/portal-visual-inspection.json`. Missing pages, clipped tables, hidden provenance/warnings, unresolved links or an unexpected evidence member block Phase 4. The package manifest records the receipt hash as a verification input; private/current-cut captures are never copied into the client ZIP.
...
**Approval:** approved 2026-09-23 after plan-review corrections for explicit continuation, parity, visual evidence and spreadsheet support boundaries
